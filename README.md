...
- Requests an initial certificate via certbot (Cloudflare DNS). You can include multiple hostnames via `CMP_CERT_DOMAINS` (comma/space separated). If DNS-01 fails, enable fallback with `CMP_CERT_HTTP_FALLBACK=1` to try HTTP-01.
...
```bash
curl -fsSL -o install.sh https://raw.githubusercontent.com/koyan-testpilot/customer-management-portal/v1.0.6/scripts/install.sh
sha256sum install.sh
